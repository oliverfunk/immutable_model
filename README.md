...
This package lets you define valid, immutable state models in a succinct way (with no code gen.) that are interoperable between commonly used state management systems and that support serialization to and from JSON out of the box.

This package supports int's, double's, bool's, String's, DateTime's, lists, enums and maps, each having their own `ModelType` representation (e.g. `ModelInt`, `ModelString`, `ModelList<T>`, `ModelEnum<E>` etc.). Maps are represented with the `ModelInner` class which defines a map between field label Strings and other `ModelType` models. The `ModelInner` is generally used for hierarchical nesting in a `ImmutableModel` (hence "inner").

Every `ModelType` can be passed a `validator` which is run when the model is updated. If the update is invalid, the instance is returned without the update applied.
...
For example, a simple authentication state model can be written as follows:
...
By parameterizing the `signIn` function with the `ModelEmail` and `ModelPassword` types, you can trust that the data these types hold is valid. Code that call it (from the UI, for example) is forced to comply and instantiate the types, which is only possible with valid data.

If it was instead written as `signIn(String email, String password)` the strings would have to be checked to comply with the validation rules. Even if you trust the UI's implementation or you trust the values that are passed in, it is still _possible_ that the values could be invalid.
...
Fully implemented examples using these ideas can be found in `/example` with the BLoC (and Cubits), Redux and MobX frameworks.

### Accessing or updating models within an `ImmutableModel`
...
A `ModelSelector<V>` can be defined and used to update a model or access its value in an `ImmutableModel` (or `ModelInner`).

A `ModelSelector` is defined using a "selector string", which use a '.' for accessing nested models.

For example, with the following state model:
